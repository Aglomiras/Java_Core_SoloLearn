...
public class Lecture_1 {
     ...
     *
     * Суть большинства компьютерных программ — создание выводов. Приведём несколько примеров:
     * - Уведомления "Вы получили новое сообщение"
     * - Текст "Game Over", который отображается на экране, когда вы играете в видеоигры
     * - Остаток денег на вашем счёте, когда вы заходите в приложение для интернет-банкинга.
     * <p>
     * Самый простой вывод — отображение сообщения на экране.
     * С помощью выводов программисты проверяют, что компьютер следует заданным инструкциям, а также исправляют ошибки в коде.
     * Следующая строка кода отображает на экране слоган Java в качестве вывода:
     * */
     ...
     * Если дать компьютеру множество инструкций, они будут выполняться построчно, сверху вниз.
     *
     * Язык Java чувствителен к регистру. Это означает, что вам нужно следить за использованием прописных и строчных букв.
     * В Java каждая строка кода, которая может выполняться, должна находиться внутри класса.
     * Вы можете задать любое имя для класса. О классах позже.
     * */
     ...
     *
     * -----float и double-----
     * По умолчанию десятичные значения имеют тип double.
     * Тип float занимает меньше места в памяти, но не так точен, как тип double.
     * Это означает, что вычисления с типом float выполняются быстрее, чем вычисления с типом double,
     * однако результат получается менее точным с точки зрения десятых.
     *
     * Общее правило следующее: использовать float вместо double, когда ресурсы памяти ограничены.
     * Если нужны более точные вычисления, например, при работе с валютой, следует использовать double.
     * */
    ...
    /**
     * Java обеспечивает богатым набором операторов для использования при манипуляции переменными.
